...
<SCRIPT LANGUAGE="JavaScript">
	function del(orderid, materialid) {
		if (confirm("确定删除该原料吗?")) {
			c = "d_dep2.jsp?del=" + orderid + "&del2=" + materialid;
			window
					.open(
							c,
							"bodyt31f",
	...
	function detail2(orderid, materialid) {
		c = "update_dep2.jsp?detail= " + orderid + " &detail2= " + materialid;
		window
				.open(
						c,
						"bodyt31f",
...
<body>
	...
	<%
		...
		try {
			...
			countrs = db1.executeQuery("select count(*) as num from order_detail where order_id = " + orderid);
		...
		String sql1 = "select * from base_order where order_id = " + orderid;
		String sql2 = "select * from order_detail where order_id =" + orderid + " limit " + (intPage - 1) * 5 + ","
				+ 5;
		String materialname = "", time = "";
		int materialid = 0, num = 0;

		ResultSet rs1 = db1.executeQuery(sql1);
		if (rs1.next()) {
			time = rs1.getString("deadline");
		}
		rs1.close();
				...
				<td><table width="100%" border="0" cellpadding="5"
						...
						<tr>
							<td colspan=5 bgcolor="#FFFFFF" align="center"><a
								HREF="javascript:detail('<%=orderid%>')">修改</a></td>
						</tr>
						<tr>
							<td colspan=5 bgcolor="#FFFFFF"><div align="center">
									<span class="STYLE10"><B><font size="5">所需原料</font></B></span>
								</div></td>
						</tr>
						<%
							while (rs2.next()) {
	...
	<%
		...
		if (intPage < 1)
			intPage = 1;
		out.print("<form method='post' name=fPageNum action='detail_dep.jsp.jsp'>");
		if (intPage < 2)
			out.print("首页 上一页  ");
		else {
			out.print("<a href='detail_dep.jsp.jsp?page=1' ><font color='#3300FF' size='2'>首页</font></a>&nbsp;");
			out.print("<a href='detail_dep.jsp.jsp?page=" + (intPage - 1)
		...
		if (intPage - intPageCount >= 0)
			...
			out.print("<a href='detail_dep.jsp.jsp?page=" + (intPage + 1)
					+ "'><font color='#3300FF' size='2'>下一页</font></a>&nbsp;<a href='detail_dep.jsp.jsp?page="
